...
`wechat-agent-lite` 有意保持为一个**工作流优先的发稿系统**。
...
这个仓库的组织方式是为了让运行时行为更容易检查和验证。

推荐的本地开发方式：

1. 先阅读相关 service、graph node 或 runtime 模块
2. 优先做聚焦修改，而不是大范围横切重写
3. 对变更模块运行 `tests/` 下的定向测试
4. 确认公开版默认值和配置模板仍然安全
5. 保持部署脚本通用，不引入个人环境假设

## 贡献说明

欢迎贡献，但前提是保持这个仓库“公开安全 + 工作流优先”的方向。
